...
typedef struct {
	...
	uint8_t		password[2];
	short			heater_temperature;	//º”»»∆˜Œ¬∂» 1Œª–° ˝
	short			dust_flow;					//1Œª–° ˝
	char			heater_switch;		//º”»»∆˜ø™πÿ
	char			vapor_calculate;	//º∆À„ÀÆ∆˚—π 0 ≤ª≤Œ”Î‘ÀÀ„  1 ≤Œ”Î‘ÀÀ„
	char			case_tmp;					//0:0°Ê 1:20°Ê 2:25°Ê
	//@Deprecated
	uint8_t		num_chn[4];
...
typedef struct {
	int					hmi_sem;
	uint16_t		lightness;		//1Œª–° ˝
	uint16_t		gray_levels;	//1Œª–° ˝
	uint8_t			off_lcd_min;		//∂® ±
	uint8_t			hand_signal_type;		//ΩÁ√Ê’˝‘⁄¥¶¿Ìµƒ–≈∫≈¿‡–Õ
	uint8_t			fd_time_task;		//lcd∂® ±πÿ±’µƒ»ŒŒÒæ‰±˙
	uint8_t			none;
}hmi_mgr_t;
